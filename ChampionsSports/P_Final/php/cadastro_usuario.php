...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $check_sql = "SELECT email FROM cadastro_usuario WHERE email = ?";
    ...
    if ($check_stmt->num_rows > 0) {
        ...
        try {
            ...
            // 2. Insere na tabela 'usuarios' para obter o id_usuarios
            $insert_user_sql = "INSERT INTO usuarios (nome, email, senha, datanascimento, telefone, cpf) VALUES (?, ?, ?, ?, ?, ?)";
            $insert_user_stmt = $conn->prepare($insert_user_sql);
            $insert_user_stmt->bind_param("ssssss", $nome, $email, $senha_hash, $datanascimento, $telefone, $cpf);
            $insert_user_stmt->execute();
            $id_usuarios_novo = $conn->insert_id;

            // 3. Insere na tabela 'cadastro_usuario' usando o id_login_novo
            $insert_cad_user_sql = "INSERT INTO cadastro_usuario (nome, cpf, data_nascimento, telefone, email, id_login) VALUES (?, ?, ?, ?, ?, ?)";
            ...
            $insert_access_sql = "INSERT INTO acessibilidade (deficiencia_visual, deficiencia_auditiva, deficiencia_motora, deficiencia_cardiaca, deficiencia_intelectual, deficiencia_neurologica, deficiencia_psiquica, outros, id_cadastro_usuario) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)";
            $insert_access_stmt = $conn->prepare($insert_access_sql);
            $insert_access_stmt->bind_param("iiiiiiiii", $deficiencia_visual, $deficiencia_auditiva, $deficiencia_motora, $deficiencia_cardiaca, $deficiencia_intelectual, $deficiencia_neurologica, $deficiencia_psiquica, $deficiencia_outros, $id_cadastro_usuario_novo);
            ...
            $_SESSION['id'] = $id_usuarios_novo; // ID da tabela 'usuarios', que é o correto para o endereço
            $_SESSION['nome'] = $nome;
            $_SESSION['id_cadastro_usuario'] = $id_cadastro_usuario_novo; // Mantemos este para outras lógicas que possam precisar

            // Redireciona para a página de endereço. Como o usuário já está logado,
            // o ID pode ser pego da sessão principal.
            header("Location: endereco.php"); // Redireciona para o formulário HTML de endereço
            exit();
