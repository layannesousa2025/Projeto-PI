...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    // Coleta e limpa os dados do formulário para segurança
    $cep = trim($_POST['cep'] ?? '');
    $uf = trim($_POST['uf'] ?? '');
    $rua = trim($_POST['rua'] ?? '');
    ...
    try {
        // Usa INSERT ... ON DUPLICATE KEY UPDATE para inserir ou atualizar o endereço.
        // Isso requer que a coluna 'id_cadastro_usuario' seja uma chave única (UNIQUE KEY) na tabela 'endereco'.
        $sql = "
            INSERT INTO endereco (id_cadastro_usuario, cep, uf, rua, bairro, cidade, complemento) 
            VALUES (?, ?, ?, ?, ?, ?, ?)
        ...
        $stmt->bind_param("issssss", $id_usuario, $cep, $uf, $rua, $bairro, $cidade, $complemento);
        $stmt->execute();
        ...
        die("Erro ao salvar o endereço: " . $e->getMessage());
    }
}
...
<body>
     ...
     <div class="card" role="region" aria-label="Formulário de endereço">
        ...
        <form id="enderecoForm" novalidate action="../php/endereco.php" method="POST">
            <div class="form-grid">
