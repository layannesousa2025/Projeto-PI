...
try {
    ...
    // A variável de sessão principal é 'id', definida no login.php
    ...
    $stmtCheck = $pdo->prepare("SELECT id_categoria FROM categoria WHERE id_cadastro_usuario = ?");
    $stmtCheck->execute([$idUsuario]);

    if ($stmtCheck->fetchColumn() === false) {
        // Se não existir, cria um novo registro para este usuário
    ...
    $stmtUpdate = $pdo->prepare("UPDATE categoria SET `$categoria` = :isFavorited WHERE id_cadastro_usuario = :idUsuario");
    $stmtUpdate->execute([
