...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($datanascimento_input) {
        ...
        if ($date && $date->format('Y-m-d') === $datanascimento_input) {
            ...
            // Melhoria: Redirecionar com mensagem de erro em vez de 'die'
            $_SESSION['error_message'] = "Formato de data inválido.";
            header("Location: cadastro.html"); // Assumindo que o formulário se chama cadastro.html
            exit;
        }
    } else {
        $_SESSION['error_message'] = "Data de nascimento não informada.";
    ...
    if ($check_stmt->num_rows > 0) {
        ...
        // Melhoria: Redirecionar de volta com uma mensagem de erro
        ...
        $insert_login_stmt = null;
        $insert_user_stmt = null;
        $insert_cad_user_stmt = null;
        $insert_access_stmt = null;
        ...
        try {
            ...
            $insert_user_sql = "INSERT INTO usuarios (nome, email, senha, datanascimento, telefone) VALUES (?, ?, ?, ?, ?)";
            $insert_user_stmt = $conn->prepare($insert_user_sql);
            $insert_user_stmt->bind_param("sssss", $nome, $email, $senha_hash, $datanascimento, $telefone);
            ...
            // Garante que todos os statements sejam fechados
            if ($insert_login_stmt) $insert_login_stmt->close();
            if ($insert_user_stmt) $insert_user_stmt->close();
            if ($insert_cad_user_stmt) $insert_cad_user_stmt->close();
            if ($insert_access_stmt) $insert_access_stmt->close();
        }
    }
    $check_stmt->close();
}
