    ...
    lista.forEach(ev => {
        ...
        card.innerHTML = `
            <div class="event-content">
                <span class="event-category">${ev.categorias_esportes}</span>
                <h3 class="event-title">${ev.tipo_eventos}</h3>
                <div class="event-info">
                    <div><i class="fas fa-calendar"></i>${dataFormatada}</div>
                    <div><i class="fas fa-map-marker-alt"></i>${ev.localizacao}</div>
                </div>
            </div>
...
function aplicarFiltros() {
    ...
    const game = gameSelect.value; // Pega o valor exato
